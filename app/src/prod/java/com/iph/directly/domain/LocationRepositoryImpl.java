...
public class LocationRepositoryImpl implements LocationRepository {
    ...
    public Observable<Location> getCurrentLocation() {
        ...
        return locationProvider.getUpdatedLocation(LocationRequest.create().setInterval(200).setNumUpdates(1)).flatMap(locations ->
                ...
                .flatMap(addresses -> {
                    Location location = new Location(addresses.get(0).getLatitude(), addresses.get(0).getLongitude(), addresses.get(0).getLocality());
                    return Observable.just(location);
                });
    }

    @Override
    public Observable<LatLng> getToiletLocation(String toiletAddress) {
        return null;
    }
}
